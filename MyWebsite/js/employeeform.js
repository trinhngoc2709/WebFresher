$(document).ready(function () {
    let employeeManage = new Employee();
    ...
    numberInputs.forEach(element => {
        element.addEventListener("keyup", (e) => {
            console.log();
            if (!employeeManage.checkValidNumber(element.value)) {
                e.target.style.borderColor = "#FF4747";
                if (element.parentElement.querySelector(".wrong-input") == undefined) {
                    ...
                    e.target.parentElement.appendChild(messageDialog);
                }
            } else {
                e.target.style.borderColor = "#019160";
                if (e.target.parentElement.querySelector(".wrong-input") != undefined)
                    document.querySelector(".wrong-input").remove();
            }
            if (e.keyCode >= 96 && e.keyCode <= 105)
                element.value = employeeManage.formatMoney(element.value);
        });
        element.addEventListener("focusout", (e) => e.target.style.borderColor = "#bbbbbb")
        element.addEventListener("focusin", (e) => e.target.style.borderColor = "#019160")
    })
})
